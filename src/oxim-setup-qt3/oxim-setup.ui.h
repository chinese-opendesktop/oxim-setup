...
void OXIM_Setup::IMListView_clicked(QListViewItem *item)
{
	...
	if (strcasecmp(imp->modname, "gen-inp") == 0 ||
	    ...
	    strcasecmp(imp->modname, "Sunpinyin") == 0 ||
...
void OXIM_Setup::IMListInit( void )
{
    ...
    oxim_init();
    VersionLabel->setText(_("Open X Input Method Setup ") + QString(oxim_version()));
	...
	item->setText(3, (imp->inuserdir ? "" : _("<built-in>")));
...
void OXIM_Setup::PropertyButton_clicked()
{
    ...
    else if (modname == "sunpinyin")
    {
	RunSunpinyin(idx, imp);
    }
}
...
/*
  * 昇陽拼音輸入法屬性
  */
void OXIM_Setup::RunSunpinyin( int idx, iminfo_t *imp )
{
    Sunpinyin form;
    settings_t *settings = NULL;
	...
	int PinyinScheme;
	/* 設定輸入法配置 */
	form.QuanPin->setChecked(true);
	form.ShuangPin->setChecked(false);
	if (oxim_setting_GetInteger(settings, "PinyinScheme", &PinyinScheme))
	{
	    if ( 1 == PinyinScheme )
	    {
		form.QuanPin->setChecked(true);
	    ...
	    {
		form.QuanPin->setChecked(false);
		form.ShuangPin->setChecked(true);
	    }
	}
    }
    ...
    settings = oxim_settings_create();
    // PinyinScheme
    oxim_settings_add_key_value(settings, "PinyinScheme",
	form.QuanPin->isChecked() ? "1" : "2");
    oxim_set_IMSettings(idx, settings);
    oxim_settings_destroy(settings);
...
void OXIM_Setup::AddIM_clicked()
{
	...
	if (file.exists())
	{
		...
		if (system(cmd.utf8().data()) == 0)
		{
		    ...
		    {
			isReload = true;
			printf(_("Convert success\n"));
		    }
		    else
		    {
			printf(_("1.Convert failure\n"));
		    }
		}
		else
		{
		    printf(_("2.Convert failure\n"));
		}
	    }
	    else if (extname == "tab")
	    {
				    ...
				    NULL, NULL))
		{
		    ...
		    {
			isReload = true;
			printf(_("Install success\n"));
		    }
		    else
		    {
			printf(_("Install failure\n"));
		    }
		}
		else
		{
		    printf(_("This kind of tab file can not be support!\n"));
		}
	    }
	    else
		printf(_("OXIM don't support this file\n"));
	}
    }
    if (isReload)
    {
...
void OXIM_Setup::DeleteIM_clicked()
{
    ...
    bool yesno = QMessageBox::warning( this, _("Remove Input Method"),
	      _("It'll remove the input method you choose from your computer,\n") +
	      _("and cannot use it again, are you sure to do?"),
              _("_YRemove"), _("_NCancel"), 0, 1, 0 );
...
void OXIM_Setup::init()
{
    setCaption(_("Open X Input Method Setting"));
    
    tabWidget->changeTab(tabWidget->page(0), _("_Input Method"));
    tabWidget->changeTab(tabWidget->page(1), _("_Fonts"));
    tabWidget->changeTab(tabWidget->page(2), _("_Phrase"));
    tabWidget->changeTab(tabWidget->page(3), _("_About"));
    
    DefaultButton->setText(_("Default"));
    QToolTip::add(DefaultButton, _("Set as default(Ctrl+Space)"));
    GlobolButton->setText(_("Gen. setting"));
    QToolTip::add(GlobolButton, _("KeyMap and Alias"));
    PropertyButton->setText(_("Property"));
    QToolTip::add(PropertyButton, _("More settings"));
    IMListView->setColumnText(0, _("circular"));
    IMListView->setColumnText(1, _("Input Method"));
    IMListView->setColumnText(2, _("Hotkey"));
    IMListView->setColumnText(3, _("=="));
    
    textLabel1->setText(_("Favor font:"));
    groupBox1->setTitle(_("Font Size(Pixels)"));
    textLabel2->setText(_("asse ar:"));
    textLabel3->setText(_("sel. ar:"));
    textLabel4->setText(_("st ar:"));
    textLabel5->setText(_("Quick mnu:"));
    textLabel6->setText(_("Symbl List:"));
    textLabel7->setText(_("XCIN Style:"));
    iconView1->findItem(trUtf8("assem. area"), Qt::CaseSensitive)->setText(_("assem. area"));
    iconView1->findItem(trUtf8("select. area"), Qt::CaseSensitive)->setText(_("select. area"));
    iconView1->findItem(trUtf8("Status Menu"), Qt::CaseSensitive)->setText(_("Status Menu"));
    iconView1->findItem(trUtf8("Quickly Menu"), Qt::CaseSensitive)->setText(_("Quickly Menu"));
    iconView1->findItem(trUtf8("Symbol List"), Qt::CaseSensitive)->setText(_("Symbol List"));
    iconView1->findItem(trUtf8("XCIN Style"), Qt::CaseSensitive)->setText(_("XCIN Style"));
   
    textLabel1_3->setText(_("You can create pharse, word or symbol at left size of table, and when you input words, you can choose to press Alt+Shift function key to input more fast."));
    
    textLabel2_2->setText(_("Author:Firefly <firefly@opendesktop.org.tw>"));
    textLabel1_2->setText(_("Website:http://opendesktop.org.tw"));
    textLabel3_2->setText(_("Copyright:"));
    
    AddIM->setText(_("Install IM"));
    QToolTip::add(AddIM, _("Install from local or network"));
    DeleteIM->setText(_("Remove IM"));
    QToolTip::add(DeleteIM, _("Remove X Input Method from local"));
